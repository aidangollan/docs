...
<ParamField body="captionOverlays" type="CaptionOverlay[]">
  Text overlays to display on the video

  Each overlay is an object:
...
<ResponseField name="data" type="Post">
  ...
  <Expandable title="Post properties">
    ...
    <ResponseField name="caption_overlays" type="CaptionOverlay[] | null">
      Text overlays to display on the video (`{ text, x, y, fontSize }` each; positions/size are fractions of the video dimensions)
    </ResponseField>
