...
Get the top N posts ranked by a specific metric (views, likes, comments, shares, or saves). Uses the latest statistics for each post. Perfect for displaying top content and identifying viral posts.

## Request Body

<ParamField body="metric" type="string" required>
  The metric to sort by. One of: `views`, `likes`, `comments`, `shares`, `saves`
</ParamField>
...
if (response.ok) {
  response.data.forEach((post, index) => {
    ...
    console.log(`   Engagement: ${post.comments + post.shares + post.saves}`);
  });
}
```
